##' Finite sample Wald test for differential phase.
##'
##' Test differential phase of circadian curve fitting using Wald test
##' @title WaldTest_diff_phase_FN
...
##' WaldTest_diff_phase_FN(tt1, yy1, tt2, yy2)


WaldTest_diff_phase_FN <- function(tt1, yy1, tt2, yy2, period = 24){
  n1 <- length(tt1)
  ...
  this_opt_commonAmp <- opt_commonPhase(tt1, yy1, tt2, yy2, period=period, parStart=beta0)
  ...
  res <- list(phase_1=par1$phase, phase_2=par2$phase, phase_c=this_opt_commonAmp[2], 
              df = dfdiff, 
              stat = stat, 
              pvalue = pvalue)
  return(res)
