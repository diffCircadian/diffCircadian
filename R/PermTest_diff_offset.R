##' Permutation test for differential basal level (vertical shift)
##'
##' Test differential basal level of circadian curve fitting using permutation test
##' @title Permutation test for detecting differential basal level
##' @import foreach
##' @import doParallel
##' @import parallel
##' @param tt1 Time vector for miltiple genes of condition 1
##' @param yy1 Expression matrix for miltiple genes of condition 1
##' @param tt2 Time vector for miltiple genes of condition 2
##' @param yy2 Expression matrix for miltiple genes of condition 2
##' @param B Number of permutation for each gene.
##' @param period Period of the since curve. Default is 24.
##' @return P-values, see details below. 
##' Formula 1: \eqn{yy = amp \times sin(2\pi/period \times (phase + tt)) + offset}
##' Formula 2: \eqn{yy = A \times sin(2\pi/period \times tt) + B * cos(2*pi/period * tt) + offset}
##' \item{pvalue}{P-value from the Permutation test}
##' @author Haocheng Ding
##' @noRd
##' @examples
##' K=100
##' n=10
##' tt1 <- seq(2,2*n,2)
##' yy1 <- matrix(0,K,n)
##' tt2 <- seq(2,2*n,2)
##' yy2 <- matrix(0,K,n)
##' for(k in 1:K){
##' set.seed(k)
##' Amp1 <- runif(1,1,2)
##' Amp2 <- runif(1,3,4)
##' Phase1 <- runif(1,0,2)
##' Phase2 <- runif(1,5,7)
##' Offset1 <- runif(1,0,3)
##' Offset2 <- runif(1,5,8)
##' yy1[k,] <- Amp1 * sin(2*pi/24 * (tt1 + Phase1)) + Offset1 + rnorm(n,0,1)
##' yy2[k,] <- Amp2 * sin(2*pi/24 * (tt2 + Phase2)) + Offset2 + rnorm(n,0,3)
##' }
##' PermTest_diff_offset(tt1, yy1, tt2, yy2, B=1000)



PermTest_diff_offset <- function(tt1,yy1,tt2,yy2,B=1000,period=24){
  numCores <- detectCores()
  registerDoParallel(numCores)
  
  K <- nrow(yy1)
  stopifnot(K == nrow(yy2))
  
  delta_offset <- rep(0,K)
  offset1 <- rep(0,K)
  offset2 <- rep(0,K)
  pvalue <- rep(0,K)
  
  
  for (k in 1:K){
    par1 <- fitSinCurve(tt1,yy1[k,])
    par2 <- fitSinCurve(tt2,yy2[k,])
    offset1[k] <- par1$offset
    offset2[k] <- par2$offset
    delta_offset[k] <- abs(offset1[k]-offset2[k])
  }
  p <- foreach (k = 1:K,.combine=c) %dopar% {
    perm_delta_offset <- rep(0,B)
    for(b in 1:B){
      set.seed(b)
      
      n1 <- length(tt1)
      n2 <- length(tt2)
      tt1_ind <- rep(1,n1)
      tt2_ind <- rep(2,n2)
      index <- c(tt1_ind,tt2_ind)
      tt12 <- c(tt1,tt2)
      index <- sample(index)
      ttmat <- cbind(tt12,index)
      perm_tt1 <- ttmat[which(index==1),1]
      perm_tt2 <- ttmat[which(index==2),1]
      
      perm_offset1 <- fitSinCurve(perm_tt1,yy1[k,])$offset
      perm_offset2 <- fitSinCurve(perm_tt2,yy2[k,])$offset
      
      
      perm_delta_offset[b] <- abs(perm_offset1-perm_offset2)
    }
    perm_delta_offset
  }
  
  for(k in 1:K){
    pvalue[k] <- sum(p>delta_offset[k])/(K*B)
  }
  return(list(pvalue=pvalue,offset1=offset1,offset2=offset2,deltaOffset=delta_offset))
}