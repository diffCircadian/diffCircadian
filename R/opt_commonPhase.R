##' Optimization for common phase exist
##'
##' Fit a joint sin curve for two conditions assuming common phase
##' @title Fit the data based on joint sin curve assuming common phase
...
##' opt_commonPhase(tt1, yy1, tt2, yy2)


opt_commonPhase <- function(tt1, yy1, tt2, yy2, period = 24, 
                          parStart = NULL){
	...
	g <- function(beta){
	  ...
	  acos_1 <- cos(w * (tt1 + phase_c))
	  acos_2 <- cos(w * (tt2 + phase_c))
	  ...
	  partial_amp_1 <- - theta_1 * sum(diffy_1 * asin_1) 
	  partial_phase_c_1 <- - amp_1 * w * theta_1 * sum(diffy_1 * acos_1) 
	  partial_phase_c_2 <- - amp_2 * w * theta_2 * sum(diffy_2 * acos_2) 
	  partial_phase_c <- partial_phase_c_1 + partial_phase_c_2
	  
	  partial_offset_1 <- - theta_1 * sum(diffy_1) 
	  partial_theta_1 <- - n1 / 2 / theta_1 + sum(diffy_1^2)/ 2 
	  
	  partial_amp_2 <- - theta_2 * sum(diffy_2 * asin_2) 
	...
	h <- function(beta){
	  ...
	  h_amp1_phasec <- theta_1 * w * sum((amp_1 * asin_1 - diffy_1) * acos_1)
	  h_amp1_offset1 <- theta_1 * sum(asin_1)
	  h_amp1_theta1 <- - sum(diffy_1 * asin_1)

	  h_phasec_phasec_1 <- theta_1 * amp_1 * w^2 * sum(amp_1 * acos_1^2 + diffy_1 * asin_1)
	  h_phasec_phasec_2 <- theta_2 * amp_2 * w^2 * sum(amp_2 * acos_2^2 + diffy_2 * asin_2)
	  h_phasec_phasec <- h_phasec_phasec_1 + h_phasec_phasec_2
	  
	  h_phasec_offset1 <- theta_1 * amp_1 * w * sum(acos_1)
	  h_phasec_theta1 <- - amp_1 * w * sum(diffy_1 * acos_1)
	  
	  h_phasec_amp2 <- theta_2 * w * sum((amp_2 * asin_2 - diffy_2) * acos_2)
	  h_phasec_offset2 <- theta_2 * amp_2 * w * sum(acos_2)
	  h_phasec_theta2 <- - amp_2 * w * sum(diffy_2 * acos_2)
	  ...
	  hmatrix[1,2] <- hmatrix[2,1] <- h_amp1_phasec
	  hmatrix[1,3] <- hmatrix[3,1] <- h_amp1_offset1
	  hmatrix[1,4] <- hmatrix[4,1] <- h_amp1_theta1
	  
	  hmatrix[2,2] <- h_phasec_phasec
	  hmatrix[2,3] <- hmatrix[3,2] <- h_phasec_offset1
	  hmatrix[2,4] <- hmatrix[4,2] <- h_phasec_theta1
	  hmatrix[2,5] <- hmatrix[5,2] <- h_phasec_amp2
	  hmatrix[2,6] <- hmatrix[6,2] <- h_phasec_offset2
	  hmatrix[2,7] <- hmatrix[7,2] <- h_phasec_theta2
	  ...
	  hmatrix[5,6] <- hmatrix[6,5] <- h_amp2_offset2
